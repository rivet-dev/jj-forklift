...
fn tracked_stacks_revset_covers_working_copy_submitted_and_frozen() {
    ...
    assert_eq!(
        revset,
        "trunk() | @:: | mutable() | trunk()..(@ | bookmarks(glob:'stack/*') | bookmarks(glob:'forklift/frozen/*'))"
    );
}
...
fn tracked_stacks_revset_trims_trailing_slash_on_prefix() {
    assert_eq!(
        ...
        "trunk() | @:: | mutable() | trunk()..(@ | bookmarks(glob:'feature/*') | bookmarks(glob:'forklift/frozen/*'))"
    );
}
...
fn ui_dry_run_uses_tracked_stacks_revset_by_default() -> anyhow::Result<()> {
    ...
    assert!(
        rendered.contains(
            "jjui -r trunk() | @:: | mutable() | trunk()..(@ | bookmarks(glob:'stack/*') | bookmarks(glob:'forklift/frozen/*'))"
        ),
